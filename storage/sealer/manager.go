...
func (m *Manager) ReacquireSectors(ctx context.Context, path string) error {
	...
	if err != nil {
		return xerrors.Errorf("YG ：reading storage metadata for %s: %w", path, err)
	}

	var meta paths.StorageConfig
	if err := json.Unmarshal(mb, &meta); err != nil {
		return xerrors.Errorf("YG ：unmarshalling storage metadata for %s: %w", path, err)
	}
	for _, v := range meta.StoragePaths {
		if err := m.localStore.OpenPath(ctx, v.Path); err != nil {
			return xerrors.Errorf("YG ：opening local path: %w", err)
		}
	}
	return nil
}
